...
import styles from './VideoTimeline.less';
...
//     };
//   }, [scale, scrollLeft, videoClips, playheadPosition]);
    ...
    videoClips.forEach(clip => {
      ...
      if (startX + width > 0 && startX < canvasRef.current!.width) {
        ...
        if (Object.keys(preloadedThumbnails).length > 0) {
            ...
            let thumbWidth = Math.min(80, availableWidth); // 最大宽度80px
...
const drawAudioWaveform = (ctx: CanvasRenderingContext2D) => {
  ...
  console.log('--audioTrackStartY---', audioTrackStartY, audioTracks)
  ...
  audioTracks.forEach(track => {
    ...
    console.log('-------width---', width, trackY)

    
    // 仅绘制可见部分
    if (startX + width > 0 && startX < canvasRef.current!.width) {
      // 计算波形数据采样间隔
      const step = Math.max(1, Math.floor(track.audioData.length / width));
      
      // 绘制中心线
      ctx.beginPath();
      ctx.moveTo(startX, trackY + audioTrackHeight / 2);
      ctx.lineTo(endX, trackY + audioTrackHeight / 2);
      ctx.strokeStyle = '#444';
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      for (let i = 0; i < width; i++) {
        const dataIndex = Math.min(Math.floor(i * step), track.audioData.length - 1);
        const amplitude = track.audioData[dataIndex] * 100;

        // 确保振幅在有效范围内
        const normalizedAmplitude = Math.max(-1, Math.min(1, amplitude));

        console.log('-------normalizedAmplitude---', normalizedAmplitude, amplitude, dataIndex)


        const x = startX + i;
        const y = trackY + audioTrackHeight / 2 - normalizedAmplitude * (audioTrackHeight / 2 - 10);
      ...
      ctx.strokeStyle = '#444';
      ctx.lineWidth = 1;
      ctx.strokeRect(0, trackY, canvasRef.current!.width, audioTrackHeight);
    }
  });
};
  ...
  reader.onload = (e) => {
    ...
    context.decodeAudioData(e.target?.result as ArrayBuffer)
      .then((buffer) => {
        ...
        // 重要：创建一个增益节点来控制音量（可以设置为0以静音）
        const gainNode = context.createGain();
        gainNode.gain.value = 0; // 设置为0以静音处理
        ...
        // 创建一个临时的JavaScript节点来捕获音频数据
        // 这种方法比直接从analyser获取更可靠
        ...
        scriptProcessor.onaudioprocess = function() {
          ...
          // 第一次获取到有效数据时保存并停止处理
          if (audioData === null && Math.max(...dataArray) > 0.01) {
            audioData = new Float32Array(dataArray);

            console.log('----11111---', audioData)
        ...
        setTimeout(() => {
          if (audioData === null) {
            // 如果没有获取到有效数据，尝试使用整个音频缓冲区的数据
            const channelData = buffer.getChannelData(0);
            // 采样数据以匹配analyser.frequencyBinCount
            const sampleRate = channelData.length / analyser.frequencyBinCount;
            const sampledData = new Float32Array(analyser.frequencyBinCount);
            
            for (let i = 0; i < analyser.frequencyBinCount; i++) {
              const index = Math.floor(i * sampleRate);
              sampledData[i] = channelData[index];
            }

            console.log('----222---', sampledData)
