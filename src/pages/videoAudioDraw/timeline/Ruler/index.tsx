...
import { IRulerScaleKey } from '../../types';
...
const Ruler = () => { 
    ...
    const drawTimeRuler = (ctx: CanvasRenderingContext2D) => {
        ...
        const scaleData = RULER_SCALE_DATA[`${scale}` as IRulerScaleKey];

        // 根据scale确定刻度间隔
        let interval = scaleData.ruleTimeStep; // 刻度间隔(秒)

        for (let sec = startSec, count = 0; sec <= endSec; sec = Number((sec + interval).toFixed(2)), count++) {
            ...
            // 绘制时间文本
            if (count % 5 === 0 || sec !== 0 && interval * scale > DEFAULT_SCALE) {
                ctx.lineTo(x, 15);
                if (sec === 0) {
                ...
                if (interval * scale > DEFAULT_SCALE) {
                    // 绘制刻度线
                    // ctx.beginPath();
                    for (let i = 1; i < 5; i ++) {
                        const subX = (sec + i * interval / 5) * scale - scrollLeft + DEFAULT_LEFT_DIS;
                        ctx.moveTo(subX, 0);
                        ctx.lineTo(subX, 10);
                    }
                }
