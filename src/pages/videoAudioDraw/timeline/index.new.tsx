...
const Timeline: React.FC<{ audioFile: File; videoId: string; videoUrl?: string; }> = ({ audioFile, videoId, videoUrl })  => { 
    // 在组件中引入Worker
        ...
        <div className={styles.timelineWrapper}>
            <div className={styles.tools}>
                ...
                <div className={styles.tool}>
                    ...
                    <Button type='primary' 
                        onClick={() => {
                            const idx = RULER_SCALE_DATA_LIST.findIndex((item) => item.scale === scale) ?? -1;
                            setScale(RULER_SCALE_DATA_LIST[Math.min(idx + 1, RULER_SCALE_DATA_LIST.length - 1)].scale);
                        }} >放大</Button>
                    <Button type='primary'
                        onClick={() => {
                            const idx = RULER_SCALE_DATA_LIST.findIndex((item) => item.scale === scale) ?? -1;
                            setScale(RULER_SCALE_DATA_LIST[Math.max(idx - 1, 0)].scale);
                        }} >缩小</Button>
                </div>
